...
Modeled after the Linux kernel coding style and SQLite's defensive practices. These supplement the universal rules above.

#### Formatting

```
Indentation:      4 spaces (no tabs)
Brace style:      K&R (see below)
Line width:       100 characters
Pointer style:    char *ptr (star binds to the variable)
```

K&R braces: opening brace on the same line for control structures, on the **next line** for function definitions. This visually separates function boundaries from flow control:

```c
/* Function definition — brace on next line */
int blob_write(BlobStore *store, const void *data, size_t len)
{
	char hash[SHA256_HEX_LEN + 1];
	int rc;

	if (!store || !data)
		return -GITEAM_EINVAL;
	...
	rc = hash_compute(data, len, hash);
	if (rc < 0)
		return rc;

	/* Single-statement bodies — no braces needed */
	for (size_t i = 0; i < store->shard_count; i++)
		shard_check(&store->shards[i]);

	return 0;
}
```

**Single-statement brace rule:** Omit braces for single-statement `if`/`for`/`while` bodies — unless any branch in an `if`/`else if`/`else` chain uses braces, in which case all branches get braces (Linux kernel rule).

#### Include Organization

Four groups, separated by blank lines. The module's own header comes first — this catches missing dependencies in that header at compile time:

```c
/* 1. This module's own header */
#include "giteam/blob_store.h"
...
Functions return `int` (0 = success, negative = error) or a pointer (`NULL` = error). Use the **goto-cleanup** pattern for any function that acquires multiple resources. This guarantees a single exit path with deterministic cleanup, no matter where failure occurs:

```c
int repo_open(const char *path, Repo **out)
{
	Repo *repo = NULL;
	char *resolved = NULL;
	int rc = -GITEAM_ERROR;
	...
	if (!repo)
		return -GITEAM_ENOMEM;

	resolved = realpath(path, NULL);
	if (!resolved)
		goto cleanup;

	repo->db = db_open(resolved);
	if (!repo->db)
		goto cleanup;
...
Define error codes as an enum with a project prefix. Provide a string conversion function:
...
const char *giteam_error_string(int code);
```

#### Memory Management

- **`sizeof(*ptr)` over `sizeof(Type)`** — survives type changes:
  ```c
  Repo *repo = calloc(1, sizeof(*repo));     /* correct */
  Repo *repo = calloc(1, sizeof(Repo));      /* fragile */
  ```
- **Zero after allocation** — prefer `calloc` or `memset(ptr, 0, sizeof(*ptr))`
- **Document ownership** in function docs: who allocates, who frees
- **Symmetry** — every `_open`/`_create`/`_new` has a matching `_close`/`_destroy`/`_free`
- **NULL after free** — `free(ptr); ptr = NULL;` prevents use-after-free
- **Arena allocators** for groups of allocations sharing a lifetime — allocate many, free once
- **Never return stack addresses** — if a function produces data, take an output pointer or heap-allocate

#### Const Correctness

Use `const` to communicate intent and catch mutation bugs at compile time:

```c
/* Input data is const — function reads but does not modify */
int blob_write(BlobStore *store, const void *data, size_t len);

/* Config is read-only in this context */
int config_get(const Config *cfg, const char *key,
               char *value_out, size_t value_len);

/* Return const when caller should not modify */
const char *giteam_version(void);
```

**Rule of thumb:** every pointer parameter is `const` unless the function needs to modify what it points to.

#### Internal Linkage

Functions and variables private to a `.c` file must be `static`. This is the C equivalent of not exporting what doesn't need to be public:

```c
/* Private — not visible outside this file */
static int validate_object_hash(const char *hash)
{
	/* ... */
}

/* Public — declared in the corresponding header */
int blob_store_write(BlobStore *store, const void *data, size_t len)
{
	if (validate_object_hash(hash) < 0)
		return -GITEAM_EINVAL;
	/* ... */
}
```

If a function isn't declared in a header, it must be `static`. No exceptions.

#### Safe String & Buffer Handling

Buffer overflows are the #1 C vulnerability. Never use unbounded operations:
...
| `gets` | `fgets` |
| `scanf("%s")` | `scanf("%255s")` with explicit width limit |

Always pass buffer sizes explicitly. Never assume a buffer is "big enough":

```c
int path_join(char *out, size_t out_len,
              const char *dir, const char *name)
{
	int n = snprintf(out, out_len, "%s/%s", dir, name);
	if (n < 0 || (size_t)n >= out_len)
		return -GITEAM_EOVERFLOW;
	return 0;
}
```
...
Uninitialized variables are undefined behavior. Always initialize:

```c
int rc = 0;
char *buf = NULL;
size_t count = 0;
TreeNode node = {0};       /* zero-initialize structs */
```

Declare variables close to first use (C11 allows this). Exception: the goto-cleanup pattern requires resource declarations before the first `goto` target.

#### Opaque Types

Hide implementation details behind opaque pointers in public headers. Callers interact through the API only — they cannot reach into struct fields:

```c
/* Public header — opaque forward declaration */
typedef struct BlobStore BlobStore;

int  blob_store_open(const char *path, BlobStore **out);
int  blob_store_write(BlobStore *store, const void *data, size_t len,
                      char *hash_out, size_t hash_len);
void blob_store_close(BlobStore *store);
```

```c
/* Source file — full struct definition, private */
struct BlobStore {
	int fd;
	char *root_path;
	size_t object_count;
...
Use `typedef struct { ... } TypeName;` for internal types that don't need to be opaque. Reserve opaque pointers for public API boundaries where encapsulation matters.

#### Header Guards

Every header uses include guards with the project prefix:

```c
#ifndef GITEAM_BLOB_STORE_H
#define GITEAM_BLOB_STORE_H

/* ... header contents ... */

#endif /* GITEAM_BLOB_STORE_H */
```

The trailing comment on `#endif` identifies which guard it closes.

#### Compiler Discipline

Always compile with maximum warnings treated as errors:
...
| `-Wstrict-prototypes` | Require full prototypes, reject K&R declarations |
| `-Wformat=2` | Catch printf/scanf format string mismatches |
| `-Wnull-dereference` | Catch potential NULL pointer dereferences (GCC) |
| `-fstack-protector-strong` | Runtime stack buffer overflow detection |

#### Assert Invariants

Following SQLite's practice: use assertions liberally to document and enforce invariants. Assertions catch logic bugs during development and serve as executable documentation:

```c
int tree_insert(Tree *tree, const char *key, const void *value)
{
	assert(tree != NULL);
	assert(key != NULL);
	assert(key[0] != '\0');           /* keys must be non-empty */
...
**Guidelines:**
- Assert preconditions at function entry for internal APIs
- Assert invariants that should **never** be false if the code is correct
- Don't assert things that can legitimately fail (file I/O, allocation, user input) — handle those with error codes
- Assertions stay on in debug builds, compiled out in release with `NDEBUG`

#### Thread Safety Documentation

Every public API function documents its thread safety in the header:

```c
/**
 * Write a blob to the content-addressed store.
 *
 * @param store     Blob store handle
 * @param data      Data to write
 * @param len       Length of data in bytes
 * @param hash_out  Buffer for resulting hash (>= SHA256_HEX_LEN + 1)
...
Use three annotations consistently: **Thread-safe**, **Reentrant**, **Signal-safe**. If the answer isn't known yet, write "TBD" — the slot's presence reminds you to determine it.

#### Logging

The goal: reconstruct *what happened and why it failed* from logs alone, without attaching a debugger. But not every line of execution — that's what debuggers are for.

**Log levels:**

| Level | When to use | Example |
|-------|-------------|---------|
| `ERROR` | Operation failed, cannot continue | `"repo_open: failed to read HEAD: %s"` |
| `WARN` | Unexpected but recoverable | `"config_load: unknown key '%s', ignoring"` |
| `INFO` | Major state transitions, operation boundaries | `"server listening on port %d"`, `"sync complete: %d objects"` |
| `DEBUG` | Decision points, branch taken, values that explain *why* | `"merge: fast-forward possible, skipping 3-way"` |
| `TRACE` | Entry/exit, raw values, wire data | `"blob_write: len=%zu hash=%s"` |

**Implementation — compile-time gating:**

DEBUG and TRACE must have zero cost in release builds. Use macros that compile out:
...
The `__attribute__((format))` lets the compiler type-check format strings even though the call goes through a variadic function.

**Log format — consistent, grep-friendly:**

```
[2026-04-18 14:32:01] ERROR blob_store.c:142: write failed: disk full
[2026-04-18 14:32:01] INFO  server.c:87: listening on port 4820
[2026-04-18 14:32:02] DEBUG sync.c:203: delta transfer: 12 objects, 4.2KB
```

Always include: timestamp, level, source location, message. Keep messages on one line so they're greppable.

**Where to log — log at boundaries and decisions:**

```c
int repo_clone(const char *url, const char *dest, CloneOpts *opts)
{
    log_info("clone: %s -> %s (role=%s)", url, dest,
             clone_role_string(opts->role));

    rc = transport_connect(url, &conn);
    if (rc < 0) {
        log_error("clone: failed to connect to %s: %s",
                  url, giteam_error_string(rc));
        return rc;
    }

    /* Log the decision, not every object */
    log_debug("clone: negotiated %d objects, %zu bytes",
              pack->object_count, pack->total_bytes);

    /* Don't log inside the per-object loop — that's TRACE territory */
    for (size_t i = 0; i < pack->object_count; i++) {
        log_trace("clone: unpacking object %zu/%zu type=%d",
                  i + 1, pack->object_count, pack->objects[i].type);
        /* ... */
    }

    log_info("clone: complete (%d objects)", pack->object_count);
    return 0;
}
```

**What NOT to log:**
- Inside tight loops at INFO or above (kills performance)
- Redundant context the caller already logged
- Sensitive data (SSH keys, auth tokens, file contents)
- Success of trivial operations (`"malloc succeeded"`)
- Raw binary data without summarizing it first

**Per-component guidance:**

| Component | INFO | DEBUG | Notes |
|-----------|------|-------|-------|
| **CLI** | Errors to stderr only | Behind `--verbose` flag | Users see this directly — keep it clean |
| **Daemon** | Every state transition, watch event, sync decision | Internal decisions, timing | Runs headless — logs are your only eyes |
| **Server** | Request log (method, path, status, duration), auth failures | Route matching, middleware decisions | Structured format for log aggregation |
| **Storage engine** | Nothing — hot path | Nothing | Log at the layer above, not inside write loops |

**The test:** if you'd add a `printf` to debug a bug, a `log_debug` should already be there.
